...
public:
  ...
  static bool parseChunkSize(std::string_view data, size_t &pos,
                             size_t &chunkSize);
...
class FileUtils {
public:
  static std::string readFile(std::string_view rootDir, std::string_view uri,
                              StatusCode &status);
  static bool writeFile(std::string_view rootDir, std::string_view uri,
                        std::string_view content, StatusCode &status);
  static bool deleteFile(std::string_view rootDir, std::string_view uri,
                         StatusCode &status);

  static std::optional<std::string> readFileContent(std::string_view filePath);
  static bool writeFileContent(std::string_view filePath,
                               std::string_view content);

  static bool isDirectory(std::string_view path);
  static std::string generateDirectoryListing(std::string_view dirPath,
                                              std::string_view uri);
