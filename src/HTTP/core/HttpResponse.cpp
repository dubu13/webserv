...
HttpResponse &HttpResponse::body(std::string_view content,
...
void HttpResponse::setHeader(
    const std::pair<std::string_view, std::string_view> &header) {
  _headers[std::string(header.first)] = header.second;
}

std::string HttpResponse::ok(std::string_view content,
                             std::string_view contentType) {
...
HttpResponse HttpResponse::file(std::string_view filePath) {
  try {
    ...
    std::string errorContent =
        "<html><body><h1>404 " + statusText + "</h1></body></html>";
    return HttpResponse()
        .status(404, statusText)
        .body(errorContent, "text/html");
  }
}

HttpResponse HttpResponse::directory(std::string_view dirPath,
                                     std::string_view uri) {
  std::string dirPathStr(dirPath);
  std::string uriStr(uri);
  return HttpResponse().status(200, "OK").body(
      FileUtils::generateDirectoryListing(dirPathStr, uriStr), "text/html");
}
