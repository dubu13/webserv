...
MethodHandler::resolvePaths(const Request &request, std::string_view root) {
  ...
  std::string filePath =
      HttpUtils::buildPath(effectiveRoot, request.requestLine.uri);
  return {effectiveRoot, filePath};
}

std::string
MethodHandler::handleGet(const Request &request, std::string_view root,
                         [[maybe_unused]] const RequestRouter *router) {
  ...
  return StaticFileHandler::handleRequest(effectiveRoot,
  ...
  if (contentTypeIt != request.headers.end()) {
    ...
    if (contentType.find("multipart/form-data") != std::string_view::npos) {
      ...
      for (const auto &file : files) {
        ...
        if (!outFile.is_open()) {
          Logger::logf<LogLevel::ERROR>("Failed to open file for writing: %s",
                                        fullPath.c_str());
          return ErrorResponseBuilder::buildResponse(500);
        }
        ...
        outFile.close();
        if (!outFile.good()) {
          Logger::logf<LogLevel::ERROR>("Failed to write uploaded file: %s",
                                        fullPath.c_str());
          return ErrorResponseBuilder::buildResponse(500);
        }
        Logger::logf<LogLevel::INFO>(
            "File uploaded successfully: %s (%zu bytes)", fullPath.c_str(),
            file.content.size());
      }
      return HttpResponse::ok("Files uploaded successfully: " + uploadedFiles,
                              "text/plain");
    }
  }
  return ErrorResponseBuilder::buildResponse(400);
...
MethodHandler::handleDelete(const Request &request, std::string_view root,
  ...
  if (!deleteResult)
    return ErrorResponseBuilder::buildResponse(500);
  Logger::logf<LogLevel::INFO>("File deleted successfully: %s",
                               filePath.c_str());
