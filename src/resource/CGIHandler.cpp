...
#include <iostream>
CGIHandler::CGIHandler(const std::string& root)
...
std::unique_ptr<HTTPResponse> CGIHandler::executeCGI(const std::string& uri, const HTTPRequest& request) {
...
std::unique_ptr<HTTPResponse> CGIHandler::executeScript(const std::string& script_path, const std::string& handler_path, const HTTPRequest& request) {
    ...
    response->setBody("CGI execution not implemented yet");
    return response;
}