...
Client::Client(int socketFd, const struct sockaddr_in &address)
    : _socketFd(socketFd), _address(address), _keepAlive(false) {
  _lastActivity = time(NULL);
}
Client::~Client() {}
Client::Client(const Client &other)
    : _socketFd(other._socketFd), _address(other._address),
      _incomingData(other._incomingData), _outgoingData(other._outgoingData),
      _keepAlive(other._keepAlive), _lastActivity(other._lastActivity) {}
...
int Client::getPort() const { return ntohs(_address.sin_port); }
...
const std::string &Client::getIncomingData() const { return _incomingData; }
const std::string &Client::getOutgoingData() const { return _outgoingData; }
void Client::appendIncomingData(const std::string &data) {
  _incomingData.append(data);
  updateActivity();
}
void Client::clearIncomingData() { _incomingData.clear(); }
void Client::setOutgoingData(const std::string &data) {
  _outgoingData = data;
  updateActivity();
}
void Client::clearOutgoingData() { _outgoingData.clear(); }
...
ssize_t Client::writeData() {
  ...
  ssize_t bytes_written =
      write(_socketFd, _outgoingData.c_str(), _outgoingData.size());
  if (bytes_written > 0) {
    _outgoingData.erase(0, bytes_written);
  }
  return bytes_written;
}
bool Client::hasDataToWrite() const { return !_outgoingData.empty(); }
