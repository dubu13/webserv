...
#include <iostream>
#include <unistd.h>
ClientHandler::ClientHandler(Server &server, const std::string &webRoot,
                             time_t timeout)
    : _server(server), _httpHandler(webRoot, &server.getConfig()),
      _timeout(timeout) {}
...
size_t ClientHandler::getClientCount() const { return _clients.size(); }
...
void ClientHandler::processRequest(int fd, Client &client) {
  try {
    ...
    client.setOutgoingData(response);
...
void ClientHandler::sendError(int fd, int status) {
  ...
  try {
    std::string response =
        HTTP::createErrorResponse(static_cast<HTTP::StatusCode>(status));
    it->second.setOutgoingData(response);
    _server.getPoller().updateFd(fd, POLLOUT);
  } catch (const std::exception &e) {
    removeClient(fd);
  }
